...
def dashboard():
    ...
    if session['role'] == 'taxpayer':
        return redirect(url_for('taxpayer'))
    elif session['role'] == 'employee':
        return redirect(url_for('employee'))
    else:
        return redirect(url_for('index'))  # Redirect to index for unknown roles
...
def login():
    ...
    flash("Неверное имя пользователя или пароль", "error1")
    return redirect(url_for('index'))  # Рендерит index.html (вход)
